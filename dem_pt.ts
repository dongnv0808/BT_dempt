let arrList: number[] = [5, 2, 2, 3, 3, 6, 6, 6, 8, 4, 4, 1, 1];
function countItem(arrList: number[], item: number): number{
    let count = 0;
...
function countBinaruSearch(arr: number[], item: number): number{
    let count = 0;
    let low = 0;
